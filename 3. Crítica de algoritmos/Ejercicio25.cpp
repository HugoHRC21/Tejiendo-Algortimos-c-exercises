...
template<typename T>
int contarNodos(Dnode<T>* low, Dnode<T>* high) {
    int count = 0;
    for (Dnode<T>* p = low; p != high->next; p = p->next) count++;
    return count;
}

template<typename T>
T seleccionarPivoteAleatorio(Dnode<T>* low, Dnode<T>* high) {
    int n = contarNodos(low, high);
    int pos = rand() % n; 
    Dnode<T>* p = low;
    while (pos--) p = p->next;
    return p->data;
...
template<typename T>
void quicksortAleatorio(Dnode<T>* head) {
    if (!head) return;
    ...
    while (!pila.empty()) {
        ...
        T pivote = seleccionarPivoteAleatorio(low, high);
